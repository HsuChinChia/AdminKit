<template>
  <Teleport to="body">
    <!-- 改為 top-5 right-5，寬度加寬到 w-96 -->
    <div class="fixed top-20 right-5 z-[9999] flex flex-col gap-3 w-96">
      <TransitionGroup name="toast">
        <div v-for="toast in notifications.toasts" :key="toast.id"
          class="flex items-start gap-4 p-5 rounded-2xl shadow-xl border-2 cursor-pointer"
          :class="toastClass(toast.type)" @click="notifications.remove(toast.id)">
          <component :is="toastIcon(toast.type)" class="w-7 h-7 shrink-0 mt-0.5" />
          <div class="flex-1 min-w-0">
            <!-- 字體變大 -->
            <p class="font-bold text-base">{{ toast.title }}</p>
            <p v-if="toast.message" class="text-sm opacity-90 mt-1">{{ toast.message }}</p>
          </div>
          <X class="w-5 h-5 opacity-60 hover:opacity-100 transition shrink-0" />
        </div>
      </TransitionGroup>
    </div>
  </Teleport>
